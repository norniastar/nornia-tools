        ...
        <div className="space-y-3">
          ...
          {formats.map((format, i) => (
            <div 
              ...
              className={`group rounded-xl bg-white border transition-all p-4 flex items-center justify-between gap-4 cursor-pointer relative overflow-hidden ${copiedKey === i ? 'border-blue-300 bg-blue-50/20' : 'border-slate-200 hover:border-blue-300 hover:bg-blue-50/30'}`}
            >
              <div className="flex items-center gap-4 w-full">
                <div className="w-8 h-8 rounded-md bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 font-bold text-xs group-hover:bg-white group-hover:text-[#0057c1] transition-colors">0{i+1}</div>
